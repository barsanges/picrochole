...
{- |
   Module      : Engine.Turn.Encirclement.Double.TestSpec
   Copyright   : Copyright (C) 2022 barsanges
   License     : GNU GPL, version 3
-}

module Engine.Turn.Encirclement.Double.TestSpec ( spec ) where
...
spec :: Spec
spec = do
  it "surroundings units may also be surrounded" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Encirclement/Double/"
    mexpected <- readUnits "test/functional/Engine/Turn/Encirclement/Double/expected-units.json"
