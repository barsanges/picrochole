...
spec = do
  it "units may be surrounded even if they are on the edge of the map" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Encirclement/Edge/"
    mexpected <- readUnits "test/functional/Engine/Turn/Encirclement/Edge/expected-units.json"
