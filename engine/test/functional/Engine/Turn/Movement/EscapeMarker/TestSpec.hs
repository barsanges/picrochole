...
spec = do
  it "units can escape a fight only if they have an eligible way out" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Movement/EscapeMarker/"
    mexpected <- readUnits "test/functional/Engine/Turn/Movement/EscapeMarker/expected-units.json"
