...
{- |
   Module      : Engine.Turn.Movement.Collision.TestSpec
   Copyright   : Copyright (C) 2022 barsanges
   License     : GNU GPL, version 3
-}

module Engine.Turn.Movement.Collision.TestSpec ( spec ) where
...
spec :: Spec
spec = do
  it "opposing units may collide if they move in the right order" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Movement/Collision/"
    mexpected <- readUnits "test/functional/Engine/Turn/Movement/Collision/expected-units.json"
