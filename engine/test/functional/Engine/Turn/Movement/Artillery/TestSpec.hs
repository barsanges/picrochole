...
{- |
   Module      : Engine.Turn.Movement.Artillery.TestSpec
   Copyright   : Copyright (C) 2022 barsanges
   License     : GNU GPL, version 3
-}

module Engine.Turn.Movement.Artillery.TestSpec ( spec ) where
...
spec :: Spec
spec = do
  it "move an artillery unit in a simple situation" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Movement/Artillery/"
    mexpected <- readUnits "test/functional/Engine/Turn/Movement/Artillery/expected-units.json"
