...
spec = do
  it "send a report each turn if the unit is really close to its HQ" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Reporting/CloseToHQ/"
    mexpected <- R.readReports "test/functional/Engine/Reporting/CloseToHQ/expected-reports.json"
