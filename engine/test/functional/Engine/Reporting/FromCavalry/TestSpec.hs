...
{- |
   Module      : Engine.Reporting.FromCavalry.TestSpec
   Copyright   : Copyright (C) 2022 barsanges
   License     : GNU GPL, version 3
-}

module Engine.Reporting.FromCavalry.TestSpec ( spec ) where
...
spec = do
  it "cavalry has a 4 hexes wide line of sight" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Reporting/FromCavalry/"
    mexpected <- R.readReports "test/functional/Engine/Reporting/FromCavalry/expected-reports.json"
