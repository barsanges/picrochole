...
spec = do
  it "send reinforcements if an enemy is in sight" $ do
    mpieces <- loadSomeDir "test/functional/Engine/IA/HQ/Reinforcements/"
    mexpected <- O.readOrders "test/functional/Engine/IA/HQ/Reinforcements/expected-orders.json"
