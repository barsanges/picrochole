...
spec = do
  it "send orders without knowing if the recipient still exists" $ do
    mpieces <- loadSomeDir "test/functional/Engine/IA/HQ/MissingRecipient/"
    mexpected <- O.readOrders "test/functional/Engine/IA/HQ/MissingRecipient/expected-orders.json"
