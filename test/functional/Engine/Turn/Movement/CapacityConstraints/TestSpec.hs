...
spec = do
  it "a unit may not advance if there is no room ahead" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Movement/CapacityConstraints/"
    mexpected <- readUnits "test/functional/Engine/Turn/Movement/CapacityConstraints/expected-units.json"
