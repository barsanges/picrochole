...
{- |
   Module      : Engine.Turn.Movement.EscapeUnit.TestSpec
   Copyright   : Copyright (C) 2022 barsanges
   License     : GNU GPL, version 3
-}

module Engine.Turn.Movement.EscapeUnit.TestSpec ( spec ) where
...
spec = do
  it "units can escape a fight only if they have an eligible way out" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Movement/EscapeUnit/"
    mexpected <- readUnits "test/functional/Engine/Turn/Movement/EscapeUnit/expected-units.json"
