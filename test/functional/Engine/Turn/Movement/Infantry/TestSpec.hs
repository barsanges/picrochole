...
{- |
   Module      : Engine.Turn.Movement.Infantry.TestSpec
   Copyright   : Copyright (C) 2022 barsanges
   License     : GNU GPL, version 3
-}

module Engine.Turn.Movement.Infantry.TestSpec ( spec ) where
...
spec :: Spec
spec = do
  it "move an infantry unit in a simple situation" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Movement/Infantry/"
    mexpected <- readUnits "test/functional/Engine/Turn/Movement/Infantry/expected-units.json"
