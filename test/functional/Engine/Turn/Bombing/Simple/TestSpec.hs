...
{- |
   Module      : Engine.Turn.Bombing.Simple.TestSpec
   Copyright   : Copyright (C) 2022 barsanges
   License     : GNU GPL, version 3
-}

module Engine.Turn.Bombing.Simple.TestSpec ( spec ) where
...
spec :: Spec
spec = do
  it "artillery has a range of 1 hex" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Bombing/Simple/"
    mexpected <- readUnits "test/functional/Engine/Turn/Bombing/Simple/expected-units.json"
