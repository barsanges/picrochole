...
{- |
   Module      : Engine.Turn.Encirclement.Simple.TestSpec
   Copyright   : Copyright (C) 2022 barsanges
   License     : GNU GPL, version 3
-}

module Engine.Turn.Encirclement.Simple.TestSpec ( spec ) where
...
spec :: Spec
spec = do
  it "units are destroyed if they are surrounded" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Encirclement/Simple/"
    mexpected <- readUnits "test/functional/Engine/Turn/Encirclement/Simple/expected-units.json"
