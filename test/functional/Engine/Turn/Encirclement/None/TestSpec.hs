...
spec = do
  it "units are destroyed only if they are surrounded" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Turn/Encirclement/None/"
    mexpected <- readUnits "test/functional/Engine/Turn/Encirclement/None/expected-units.json"
