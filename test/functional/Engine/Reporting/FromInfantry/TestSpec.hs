...
spec = do
  it "infantry has a 2 hexes wide line of sight" $ do
    mpieces <- loadSomeDir "test/functional/Engine/Reporting/FromInfantry/"
    mexpected <- R.readReports "test/functional/Engine/Reporting/FromInfantry/expected-reports.json"
